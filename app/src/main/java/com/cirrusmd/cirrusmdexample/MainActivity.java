...
public class MainActivity extends AppCompatActivity implements CirrusListener {
    ...
    public void onBackPressed() {
        int supportCount = getSupportFragmentManager().getBackStackEntryCount();

        if (supportCount > 0) {
            Toast.makeText(this, "onBackPressed not overridden", Toast.LENGTH_SHORT).show();
            super.onBackPressed();
        } else {
            Toast.makeText(this, "onBackPressed overridden", Toast.LENGTH_SHORT).show();
        }
    }
